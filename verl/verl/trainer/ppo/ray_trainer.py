...
#                     non_tensor_batch_keys_to_pop.append("tools_kwargs")
#                 if "origin_ques" in batch.non_tensor_batch:
#                     non_tensor_batch_keys_to_pop.append("origin_ques")
...
#                             batch, kl_metrics = apply_kl_penalty(batch, kl_ctrl=self.kl_ctrl_in_reward, kl_penalty=self.config.algorithm.kl_penalty, multi_turn=self.config.actor_rollout_ref.rollout.multi_turn.enable or self.config.actor_rollout_ref.rollout.learn2ask)
...
#                             norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
#                             multi_turn=self.config.actor_rollout_ref.rollout.multi_turn.enable or self.config.actor_rollout_ref.rollout.learn2ask,
...
#                             batch.meta_info["multi_turn"] = self.config.actor_rollout_ref.rollout.multi_turn.enable or self.config.actor_rollout_ref.rollout.learn2ask
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "tools_kwargs" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.append("tools_kwargs")
            if "origin_ques" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.append("origin_ques")
            test_gen_batch = test_batch.pop(
                batch_keys=batch_keys_to_pop,
                non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    print(timing_raw)
                    print("Now is recomputing old_log_probs ... ")
                    # recompute old_log_probs
                    with _timer("old_log_prob", timing_raw):
                        old_log_prob = self.actor_rollout_wg.compute_log_prob(batch)
                        entropys = old_log_prob.batch["entropys"]
                    ...
                    print(timing_raw)
                    print("Now is recomputing ref_log_probs ... ")
                    if self.use_reference_policy:
                        # compute reference log_prob
                        with _timer("ref", timing_raw):
                            if not self.ref_in_actor:
                    ...
                    print(timing_raw)
                    print("Now is computing advantages ... ")
                    with _timer("adv", timing_raw):
                        # we combine with rule-based rm
                        reward_extra_infos_dict: dict[str, list]
                        if self.config.reward_model.launch_reward_fn_async:
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            batch, kl_metrics = apply_kl_penalty(
                                ...
                                or self.config.actor_rollout_ref.rollout.learn2ask,
                            )
                            metrics.update(kl_metrics)
                        else:
