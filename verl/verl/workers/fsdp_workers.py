...
#                 from verl.workers.rollout.vllm_rollout import L2AvLLMRollout
#                 vllm_rollout_cls = vLLMRollout if self.config.rollout.mode == "sync" else vLLMAsyncRollout
#                 vllm_rollout_cls = L2AvLLMRollout if self.config.rollout.learn2ask else vLLMRollout
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            if vllm_mode == "customized":
                ...
                vllm_rollout_cls = (
                    L2AvLLMRollout if self.config.rollout.learn2ask else vLLMRollout
                )
                # vllm_rollout_cls = L2AvLLMRollout
                print(f"+++++++++++++++++{vllm_rollout_cls}++++++++++++++++++++++++++")
                rollout = vllm_rollout_cls(
                    model_path=local_path,
                    config=self.config.rollout,
                    tokenizer=self.tokenizer,
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            print("fsdp_wokers.py Line 883  Now is preprocessing")
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)

            print("fsdp_wokers.py Line 886  Now is generating")
            output = self.rollout.generate_sequences(prompts=prompts)

            log_gpu_memory_usage("After rollout generation", logger=logger)

            print("fsdp_wokers.py Line 891  Now is postprocessing")
            output = self.rollout_sharding_manager.postprocess_data(output)
        ...
        print("fsdp_wokers.py Line 899  Now is Returning")
        return output
