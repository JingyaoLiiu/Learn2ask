    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self.sampling_params.n > 1 and do_sample:
            ...
            if "origin_ques" in non_tensor_batch.keys():
                non_tensor_batch["origin_ques"] = _repeat_interleave(non_tensor_batch["origin_ques"], self.sampling_params.n)
        ...
        def call_user_agent(response_text,context):       
            ...
            prompt = PROMPT_TEMPLATE.replace("[Context]", context).replace("[Question]", extracted)
            ...
            try:
                ...
                res = requests.post('http://localhost:8080/v1/chat/completions', json=payload, timeout=10)
        ...
        def return_response_and_mask(tokenizer, messages):
            ...
            last_token_ids = tokenizer.apply_chat_template(fix_messages(session), tokenize=True, add_generation_prompt=True, enable_thinking=True)
            for message in messages[1:]:
                session.append(message)
                token_ids = tokenizer.apply_chat_template(fix_messages(session), tokenize=True, add_generation_prompt=message["role"] == "user", enable_thinking=False)
                ...
                if message["role"] == "assistant":
                    return_label_mask += [1] * (len(token_ids) - len(last_token_ids) - 1) + [0]
                else:
                    return_label_mask += [0] * (len(token_ids) - len(last_token_ids))
                last_token_ids = token_ids 
            return return_response[:-1], return_label_mask[:-1] # final token is always \n
        ...
        messages_list = [[{"role": "user", "content": extract_qwen_input(self.tokenizer.decode(vllm_input["prompt_token_ids"]))}] for vllm_input in vllm_inputs]
        ...
        with self.update_sampling_params(**kwargs):
            ...
            [update_messages(messages, next_turn) for messages, next_turn in zip(messages_list, responses)]

            tmp_message_list = [] 
            tmp_origin_ques_list = []

            # for messages in messages_list:
            for messages, origin_ques in zip(messages_list, non_tensor_batch["origin_ques"]):
                final_turn_content = messages[-1]["content"]
                boxed = extract_text(extract_boxed(final_turn_content))
                if boxed and ("?" in boxed[-3:] and len(boxed.split()) > 3): # is a question
            ...
            if tmp_message_list:
                ...
                [update_messages(messages, next_turn) for messages, next_turn in zip(tmp_message_list, user_replies)]
            ...
            if tmp_message_list:
                vllm_inputs = [{"prompt_token_ids": self.tokenizer.apply_chat_template(fix_messages(messages), tokenize=True, add_generation_prompt=True, enable_thinking=False)} for messages in tmp_message_list]
                ...
                responses = [output.outputs[0].text for output in outputs]
                [update_messages(messages, next_turn) for messages, next_turn in zip(tmp_message_list, responses)]

            response, label_mask = [], []
            for messages in messages_list:
                return_response, return_label_mask = return_response_and_mask(self.tokenizer, messages)
            ...
            response = pad_2d_list_to_length(response, self.pad_token_id, max_length=self.config.response_length * 2).to(idx.device)
            label_mask = pad_2d_list_to_length(label_mask, 0, max_length=self.config.response_length * 2).to(idx.device)
        ...
        response_attention_mask = get_response_mask(response_id=response, eos_token=self.pad_token_id, dtype=attention_mask.dtype)
