...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: Union[str, List[str]], tokenizer, config=None):
        ...
        if "qwen" in str(tokenizer.__class__).lower():
    ...
    def _process_message_tokens(
        ...
        # for llama mask special token like <|eot_id|>
        special_tokens_mask = self.tokenizer.get_special_tokens_mask(
            message_tokens, already_has_special_tokens=True
        )
        loss_mask = [
            0 if is_special else lm for is_special, lm in zip(special_tokens_mask, loss_mask)
        ]
