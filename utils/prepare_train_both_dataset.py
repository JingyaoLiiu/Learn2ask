...
data_path = "/public/home/ldk/users/wat/learn2ask/dataset/full_train_data_search.jsonl"
...
for instance in tqdm(dataset):
    ...
    null = 0
    total = 0
    if len(instance["rollouts"]) < 5:
        continue
    for rollout in instance["rollouts"]:
        if "correct" in rollout:
            if rollout["correct"]:
                corr += 1
            total += 1
    if corr > 0 and corr < total:
        output.append(process_fn(instance))
        if total not in total_list:
            total_list[total] = 0
        total_list[total] += 1
    if corr == 0 and 4 < total < 15:
        output.append(process_fn(instance))

# diverse
# print(len(set(item["origin"] for item in output)))
print(len(output), len(dataset))
print([(key, total_list[key]) for key in sorted(total_list)])

with jsonlines.open(f"train_both.jsonl", "w") as writer:
    writer.write_all(output)

output = Dataset.from_list(output)
output.to_parquet(f"train_both.parquet")